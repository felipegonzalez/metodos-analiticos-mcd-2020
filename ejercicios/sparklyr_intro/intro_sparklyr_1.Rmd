...
escalar nuestro análisis (tanto a máquinas con muchos cores y memoria como a clusters para
los problemas más grande). En primer lugar, notamos que cuando es posible,
es más simple y rápidos usar máquinas más grandes y trabajar con las herramientas
más flexibles que conmunmente utilizamos (pandas y dplyr), ver por ejemplo [aquí](https://h2oai.github.io/db-benchmark/).
...
Nótese que aquí hicimos una partición de los datos explícita. Cuando transformamos o 
...
```{r}
perfiles_tbl %>% group_by(age) %>% tally()
```
...
ni ha traido los datos a memoria.
...
- Los estimadores usuales de Spark comienzan con *ml_*, como 
...
Una de las operaciones más costosas es el *shuffle* que es necesario cuando se tienen
que enviar entre trabajadores. Ejemplos usuales son *group_by*, *joins*, *repartition* (según
alguna variable). 
...
```{r}
spark_write_parquet(perfiles, path = "../../datos/profiles.parquet",
                    partition_by = "a_mes")
```
