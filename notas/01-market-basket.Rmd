...
library(tidygraph)
library(ggraph)
df_reglas <- reglas_f %>% DATAFRAME %>% rename(from=LHS, to=RHS) %>% as_tibble
...
```{r}
library(readr)
b_reglas_lift <- subset(b_reglas, hyper_lift > 1.5 & lhs.support > 0.01)
reglas_f <- subset(b_reglas_lift, size(b_reglas_lift)==2)
length(reglas_f)
write_csv(reglas_f %>% DATAFRAME %>% 
          rename(source=LHS, target=RHS) %>% select(-count), 
