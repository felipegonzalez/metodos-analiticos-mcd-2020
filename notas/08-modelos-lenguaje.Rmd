...
En algunos casos, estos modelos del lenguaje están diseñados para
resolver ciertas tareas particulares y estimar probabilidades particulares
...
y supongamos que tenemos sólo tres frases en nuestro lenguaje:
...
**Observación**: para que este modelo sea realmente una distribución de probabilidad
sobre todas las frases posibles, es necesario incluir símbolos  final de frases,
por ejemplo  $</s>$, pues si no tenemos problemas con la longitud de las frases.
Por ejemplo, supongamos que nuestro vocabulario tiene solo dos palabras a y b. Si no tenemos
marcadores de final de frases, entonces tenemos que $P(a) + P(b) = 1$. Esto implica
que la suma de las probabilidades de las frases de longitud 1 es 1. Igualmente, la suma
de las probabilidades de las frases de longitud 2 es 1 (demuestralo).

Sin embargo, supongamos que incluímos símbolo de inicio y final de frase, de modo que
$P(a) + P(b) +  P(</s>) = 1$. Usamos la notación  $p_f = P(</s>)$. 

Entonces la suma de las probabilidades de frases de tamaño 1
es
$$P(a)P(\mathcal </s\mathcal >)+P(b)P(\mathcal </s\mathcal >)$$
que es igual a
$$p_f (1 - p_f)$$

la suma de frases de tamaño 2 es

$$\sum_{w,z} P( w z \mathcal</s\mathcal>) = p_f\sum_{w,z}(P(w)P(z))$$
que es igual a
$$p_f(1-p_f)^2$$
y así sucesivamente. Esta es una suma geométrica que suma a 1 (incluyendo frases de longitud 0):
$$\sum_{k=0}^\infty p_f (1-p_f)^k=1$$
y así el modelo de unigramas una distribución de probabilidad sobre todas las frases posibles
de este vocabulario.
...
Para bigramas, necesitamos usar también un símbolo para el final de frase, y es conveniente
también usar un símbolo de inicio de frase, por ejemplo $<s>$. De esta forma, la 
probabilidad de que el inicio de frase ocurra en la primera
posición siempre es 1, y las probabilidades suman 1, y escribimos simplemente:

$$P(w_1\cdots w_n)  \propto  P(w_2|w_1)P(w_3|w_2)  P(w_4|w_3) \cdots P(w_{n}|w_{n-1}).$$
donde $w_1=<s>$ es el inicio de frase y $w_n=</s>$ es el final de frase.
...
$$P(w_1\cdots w_n) \propto P(w_3|w_1w_2)  P(w_4|w_2w_3) \cdots P(w_n| w_{n-2} w_{n-1}).$$
...
conteo_ngramas <- function(corpus, n = 1, vocab_df = NULL){
  ...
  ngramas_df <- corpus %>%
                mutate(txt = paste(inicio, txt, " _ss_")) %>%
