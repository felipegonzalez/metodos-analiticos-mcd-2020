...
library(textreuse)
set.seed(823)
minhashes_funs <- minhash_generator(50)
```
...
corpus_tweets <- TextReuseCorpus(text = tw[1:200000], 
    ...
    hash_func = minhashes_funs, keep_tokens = TRUE,
...
map(names(indices), ~ corpus_tweets[[.x]]$content)[1:10]
```
...
para los elementos similares resultantes, y filtrar adicionalmente los
elementos que tengan similitud baja. 
...
Ahora podemos extraer los pares de similitud alta. Nótese que creamos todos los pares dentro 
de cada cubeta, y luego quitamos todos los pares repetidos:
...
DT::datatable(extraer_pares(candidatos, cubeta_2, docs, textos = textos_dup))
```
...
## Ejemplo: deduplicar tweets con textreuse

En primer lugar, podemos deduplicar tweets idénticos. Esto lo podemos hacer con una función hash simple

```{r, collapse=TRUE}
tw_hashes <- digest::digest2int(tw)
tw_dedup <- tibble(tweet = tw, hash = tw_hashes) %>% 
  group_by(hash) %>% 
  summarise(tweet = tweet[1]) %>% pull(tweet)
length(tw)
length(tw_dedup)
```
...
```{r}
set.seed(834)
minhash_funs <- minhash_generator(12)
corpus_tweets <- TextReuseCorpus(text = tw_dedup, 
    tokenizer = calcular_tejas, k = 4,
    minhash_func = minhash_funs, hash_func = minhash_funs,
    keep_tokens = TRUE,
    skip_short = TRUE, progress = FALSE)
```

```{r}
#Hacemos 3 bandas de 4 renglones (hashes)
cubetas_tbl <- lsh(corpus_tweets, bands = 3, progress = FALSE)
cubetas_tbl
```

```{r}
cubetas_tbl_ag <- cubetas_tbl %>% group_by(buckets) %>% 
  summarise(docs = list(doc), n_docs = length(doc)) %>% 
  arrange(desc(n_docs))
cubetas_tbl_ag
```
...
```{r}
lapply(cubetas_tbl_ag$docs[[1]], function(x) {corpus_tweets[[x]]$content}) %>% head
lapply(cubetas_tbl_ag$docs[[2]], function(x) {corpus_tweets[[x]]$content}) %>% head
lapply(cubetas_tbl_ag$docs[[190]], function(x) {corpus_tweets[[x]]$content}) %>% head
```

Podemos calcular la similitud de jaccard exacta entre candidatos, y filtrar los que tengan similitud baja,
por ejemplo

```{r}
candidatos <- lsh_candidates(cubetas_tbl)
candidatos
cand_score <- lsh_compare(candidatos, corpus_tweets, jaccard_similarity)
cand_score
qplot(cand_score$score)
```


## Ejemplo: encontrar vecinos cercanos

```{r, message=FALSE, warning=FALSE}
# encontrar firma minhash
textos_nuevos <- c("Croatia broke through to finals", "One of these in the final Russia Croatia Sweden Switzerland")
tweet <- TextReuseCorpus(
    text = textos_nuevos, 
    tokenizer = calcular_tejas, k = 4,
    minhash_func = minhash_funs, hash_func = minhash_funs,
    keep_tokens = TRUE,
    skip_short = TRUE, progress = FALSE)
# encontrar las cubetas
cubetas <- lsh(tweet, bands = 3, progress = FALSE)$buckets
cubetas
```

```{r}
vecinos <- cubetas_tbl %>% filter(buckets %in% cubetas) %>% pull(doc)
texto_vecinos <- map(vecinos, ~ corpus_tweets[[.x]]$content)
```

Podemos finalemente calcular similitud para descartar falsos positivos. Por ejemplo,

```{r}
sims <- map_dbl(texto_vecinos, ~ sim_jaccard(calcular_tejas(.x, 5), 
            calcular_tejas(textos_nuevos[2], 5)))
tibble(b = unlist(texto_vecinos), score = sims) %>% 
  filter(score > 0.4) %>% DT::datatable()
```


```{r}
sims <- map_dbl(texto_vecinos, ~ sim_jaccard(calcular_tejas(.x, 5), 
            calcular_tejas(textos_nuevos[1], 5)))
tibble(b = unlist(texto_vecinos), score = sims) %>% 
  filter(score > 0.3) %>% DT::datatable()
```
...
Podemos escribir una implementación de este algoritmo como siuge:
...
```{r}
sim_aprox <- apply(firmas, 2, function(x) mean(firmas[,1] == x))
length(which(sim_aprox > 0.4))
```
