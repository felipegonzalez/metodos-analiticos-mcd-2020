...
```{r}
#Hacemos 2 bandas de 6 renglones (hashes)
cubetas_tbl <- lsh(corpus_tweets, bands = 2, progress = TRUE)
cubetas_tbl
```
...
tweet <- TextReuseCorpus(
    text = c("Croatia is through finally", "France is in the World Cup"), 
    tokenizer = calcular_tejas, k = 5,
    minhash_func = minhash_funs, hash_func = minhash_funs,
    keep_tokens = TRUE,
    skip_short = TRUE)
# encontrar las cubetas
cubetas <- lsh(tweet, bands = 2, progress = TRUE)$buckets
cubetas
```
...
sims <- map_dbl(texto_vecinos, ~ sim_jaccard(calcular_tejas(.x, 5), 
            calcular_tejas("Croatia is through finally", 5)))
tibble(a = "Croatia is through finally", b = unlist(texto_vecinos), score = sims)
```
